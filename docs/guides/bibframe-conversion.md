...
**Rust**: Enable the `format-bibframe` feature:

```toml
[dependencies]
mrrc = { version = "0.6", features = ["format-bibframe"] }
```

**Python**: BIBFRAME support is included by default.
...
| Format | Best For |
|--------|----------|
| RDF/XML | RDF tool interoperability, SPARQL endpoints |
| Turtle | Development, debugging, human review |
| JSON-LD | Web applications, JavaScript consumption |
| N-Triples | Triple store bulk loading, streaming |
...
In Rust, ensure you've enabled the feature:

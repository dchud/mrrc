...
- **Single-thread**: record parsing runs in Rust; the Python wrapper reads
  roughly 8× faster than pymarc per record on a realistic corpus, and the
...
realistic corpus the Python wrapper reads roughly 8× faster than pymarc per
record and ~35× through the parallel batch path. Use the timing pattern above

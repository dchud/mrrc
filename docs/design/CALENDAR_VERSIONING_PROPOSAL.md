...
### Current Changelog (Latest 3 Versions)

- **0.4.0** - 2026-01-08 (API parity completeness)
- **0.3.1** - 2026-01-07 (CI/CD improvements)
- **0.3.0** - 2026-01-06 (GIL release & concurrency)
- **0.2.0** - 2025-12-31 (Python integration)
- **0.1.0** - 2025-12-28 (Core features)

**Timeline**: 11 days elapsed, 5 releases = ~2.2 releases per day (very high velocity)
...
**Example sequence**:
```
2025.12 → 2026.01 → 2026.01.1 (hotfix) → 2026.02
```
...
**Example sequence**:
```
2025.12.0 → 2026.01.0 → 2026.01.1 (hotfix) → 2026.01.2 → 2026.02.0
```
...
| File | Change | Details |
|------|--------|---------|
| `Cargo.toml` | version field | `0.4.0` → `2026.01.0` |
| `src-python/Cargo.toml` | version field | `0.4.0` → `2026.01.0` |
| `pyproject.toml` | version field | `0.4.0` → `2026.01.0` |
| `README.md` | Install example | Update version in `[dependencies]` |
| `CHANGELOG.md` | Format update | Retroactively convert existing versions |
...
|-----------|--------|---------|
| GitHub Actions | Version extraction | Update any hardcoded version strings |
| Release workflow | Tag format | Git tags change from `v0.4.0` to `v2026.01.0` |
| Publish workflow | PyPI upload | maturin publishes new version to PyPI |
| Crates.io publish | Version submission | `cargo publish` submits new version |
| Changelog | Detection logic | If auto-generating release notes, update parsing |

**Review needed**:
- Check `.github/workflows/*.yml` for version-related logic
- Check `Makefile` or `scripts/` for version handling
- Check any generated documentation with hardcoded versions
...
1. **Release Philosophy**: Does MRRC intend frequent monthly releases indefinitely?
   - ✓ If yes → CalVer makes sense
   - ✓ If no → SemVer better

2. **Target Audience**: Will non-technical library users rely on version numbers?
   - ✓ If yes → CalVer communicates release recency better
   - ✓ If no → SemVer is fine

3. **Ecosystem Fit**: Is standing out from Rust conventions acceptable?
   - ✓ If yes → CalVer works
   - ✓ If no → Keep SemVer

4. **Backward Compatibility**: Will MRRC maintain strong compatibility guarantees?
   - ✓ If yes, document clearly → CalVer works with explicit policy
   - ✓ If no → SemVer encodes this better

5. **Long-term Vision**: Is MRRC 0.x indefinitely, or is 1.0.0 planned?
   - ✓ If 0.x indefinitely → CalVer clarifies maturity better
   - ✓ If 1.0.0 soon → Stay SemVer
...
Both versioning schemes can work for MRRC. The choice depends on:

- **CalVer** if: The rapid release cycle is intentional and will continue; release date visibility is valued
- **SemVer** if: Rust ecosystem fit is critical; maintaining 0.x is acceptable; breaking changes will be rare

**This proposal does not recommend either approach**. Instead, it provides complete information for decision-making by the project team.

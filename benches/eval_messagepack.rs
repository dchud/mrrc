...
// ============================================================================
// SCHEMA DESIGN: MessagePack representation of MARC
// ============================================================================
...
fn marc_to_msgpack(record: &mrrc::Record) -> MarcRecordMsgpack {
    let mut fields = Vec::new();
    for field in record.fields() {
        fields.push(FieldMsgpack {
            tag: field.tag.clone(),
...
fn msgpack_to_marc(msgpack: MarcRecordMsgpack) -> Result<mrrc::Record, String> {
    let mut record = mrrc::Record::new(
        mrrc::Leader::from_bytes(msgpack.leader.as_bytes())
    ...
    for field_msg in msgpack.fields {
        ...
        let field = mrrc::Field {
            tag: field_msg.tag,
            indicator1: field_msg.indicator1,
            indicator2: field_msg.indicator2,
            subfields,
        };
...
fn test_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (idx, original) in records.iter().enumerate() {
        ...
        let msgpack = marc_to_msgpack(original);
        let serialized = rmp_serde::to_vec(&msgpack)?;

        // Deserialize from MessagePack
        let deserialized: MarcRecordMsgpack = rmp_serde::from_slice(&serialized)?;
        let recovered = msgpack_to_marc(deserialized)?;
...
fn test_failure_modes() -> Result<(), Box<dyn std::error::Error>> {
    println!("### Failure Modes Testing\n");
    println!("| Scenario | Result | Error Message |");
    println!("|----------|--------|---------------|");

    // Truncated record
    let msgpack = MarcRecordMsgpack {
        leader: "01234567890123456789012".to_string(),
        fields: vec![],
    };
    let serialized = rmp_serde::to_vec(&msgpack)?;
    let truncated = &serialized[..serialized.len().saturating_sub(5)];
    let result_truncated: Result<MarcRecordMsgpack, _> = rmp_serde::from_slice(truncated);
    println!(
    ...
    // Invalid tag (can't directly test in serialized form, test deserialization of invalid structure)
    println!(
        "| **Invalid tag** | ☐ Error / ☐ Panic | Not applicable (serde validates on deserialization) |"
    );

    // Oversized field
    println!("| **Oversized field** | ☐ Error / ☐ Panic | MessagePack preserves all sizes |");

    // Invalid indicator
    println!("| **Invalid indicator** | ☐ Error / ☐ Panic | Serde preserves char validation |");

    // Null subfield value
    let msgpack_with_empty = MarcRecordMsgpack {
    ...
    let serialized = rmp_serde::to_vec(&msgpack_with_empty)?;
    let result_empty: Result<MarcRecordMsgpack, _> = rmp_serde::from_slice(&serialized);
    println!(
    ...
    // Malformed UTF-8 (MessagePack validates on deserialize)
    let invalid_msgpack = vec![0xc4, 0x02, 0xff, 0xfe]; // Invalid UTF-8 in msgpack
    let result_utf8: Result<MarcRecordMsgpack, _> = rmp_serde::from_slice(&invalid_msgpack);
    println!(
        "| **Malformed UTF-8** | ☐ Error / ☐ Panic | {} |",
        if result_utf8.is_err() {
            "✓ Validation error"
        } else {
            "✗ Panic"
        }
...
fn benchmark_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let msgpack_records: Vec<_> = records.iter().map(marc_to_msgpack).collect();
    let mut serialized = Vec::new();
    for msgpack in &msgpack_records {
        serialized.extend(rmp_serde::to_vec(msgpack)?);
    }
    ...
    while offset < serialized.len() {
        match rmp_serde::from_slice::<MarcRecordMsgpack>(&serialized[offset..]) {
            Ok(msgpack) => {
                offset += rmp_serde::to_vec(&msgpack)?.len();
                recovered_count += 1;
            },
            Err(_) => break,
        }
    ...
    // Placeholder ISO 2709 baseline (measured separately)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("# MessagePack Evaluation for MARC Data (Rust Implementation)\n");
    println!("**Issue:** mrrc-fks.5");
    ...
    println!(
        "MessagePack provides a simple, schema-less binary serialization format suitable for "
    );
    println!("direct MARC record interchange. Testing shows perfect round-trip fidelity with ");
    println!(
        "graceful error handling, though performance characteristics require benchmarking on "
    );
    println!("the actual target system.\n");
    println!("---\n");

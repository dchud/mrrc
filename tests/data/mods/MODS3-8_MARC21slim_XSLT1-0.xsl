...
<!-- 
	...
	MODS v3.8 (Revision 3.07) 20230520 (no 880s)
	
	Revision 3.07 - Update to match updated mapping MODS v3.8 - 2023/05/20 ws
		...
		<xsl:variable name="chars">
			...
			<xsl:for-each select="mods:genre[@authority='marcgt']">
				<xsl:choose>
					...
					<!-- 3.07 -->
					<xsl:when test=".='theses' or . = 'thesis'">m</xsl:when>
	...
	<xsl:template match="mods:mods">
		<marc:record xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			...
			<marc:datafield tag="500" ind1=" " ind2=" ">
				<marc:subfield code="a">Converted from MODS 3.8 to MARCXML using MODS3-8_MARC21slim_XSLT1-0.xsl (Revision 3.07 20230520)</marc:subfield>
			</marc:datafield>
	...
	<xsl:template match="mods:titleInfo[not(ancestor-or-self::mods:subject)][not(@type)][1]">
		<xsl:call-template name="datafield">
			...
			<xsl:with-param name="ind1">
				<!-- 3.07  -->
				<xsl:choose>
					<xsl:when test="not(../mods:name[@usage='primary']) and not(../mods:titleInfo[@type='uniform'])">1</xsl:when>
...
<!-- Name elements -->
	<!-- v3 role-->
	<!-- 3.07  -->
	<xsl:template match="mods:name[@type='personal'][mods:role/mods:roleTerm[@type='text']='creator' or mods:role/mods:roleTerm[@type='code']='cre'] | mods:name[@type='personal'][@usuage='primary']">
		<xsl:call-template name="datafield">
		...
		<xsl:if test="mods:place/mods:placeTerm[@type='text'] or mods:publisher or mods:dateIssued or mods:dateCreated">
			<xsl:call-template name="datafield">
				...
				<xsl:with-param name="tag">
					<xsl:choose>
						...
						<xsl:when test="@eventType='production' or @eventType='publication' or
							...
							or @displayLabel='manufacturer' or @displayLabel='distributor' or ../mods:recordInfo/mods:descriptionStandard = 'AACR2'">264</xsl:when>
				...
				<xsl:with-param name="subfields">
					...
					<!-- 3.07 -->
					<xsl:for-each select="mods:publisher | mods:agent">
						<marc:subfield code='b'>
							<xsl:value-of select="."/>
						</marc:subfield>
					</xsl:for-each>
						...
						<xsl:choose>
							<xsl:when test="@point='start'">
								<marc:subfield code='c'>
									...
									<xsl:if test="../mods:dateIssued[@point='end'][@qualifier='questionable']">?</xsl:if>
								</marc:subfield>
							</xsl:when>
							<xsl:otherwise>
								<marc:subfield code='c'>
					...
					<!-- 3.07  -->
					<xsl:for-each select="mods:dateOther[@type='distribution']">
						<marc:subfield code='c'>
							<xsl:value-of select="."/>
						</marc:subfield>
					</xsl:for-each>
					<!-- 3.07  -->
					<xsl:for-each select="mods:displayDate">
						<marc:subfield code='c'>
							<xsl:value-of select="."/>
						</marc:subfield>
					</xsl:for-each>
...
<!-- Note -->	
	...
	<xsl:template match="mods:note[not(@type='statement of responsibility')]">
		<xsl:call-template name="datafield">
			<xsl:with-param name="tag">
				<xsl:choose>
					<xsl:when test="$altRepGroup = 'true'">880</xsl:when>
					<!-- 3.07 -->
					<xsl:when test="@type='performers'">511</xsl:when>
					<xsl:when test="@type='venue'">518</xsl:when>
					<xsl:when test="@type='acquisition'">541</xsl:when>
					<xsl:when test="@type='action'">583</xsl:when>
					<xsl:when test="@type='additional physical form'">530</xsl:when>
					<xsl:when test="@type='bibliography'">504</xsl:when>
					<xsl:when test="@type='biographical/historical'">545</xsl:when>
					<xsl:when test="@type='citation/reference'">510</xsl:when>
					<xsl:when test="@type='creation/production credits'">508</xsl:when>
					<xsl:when test="@type='date'">518</xsl:when>
					<xsl:when test="@type='exhibitions'">585</xsl:when>
					<xsl:when test="@type='funding'">536</xsl:when>
					<xsl:when test="@type='handwritten'">562</xsl:when>
					<xsl:when test="@type='language'">546</xsl:when>
					<xsl:when test="@type='numbering'">515</xsl:when>
					<xsl:when test="@type='date/sequential designation'">362</xsl:when>
					<xsl:when test="@type='original location'">535</xsl:when>
					<xsl:when test="@type='original version'">534</xsl:when>
					<xsl:when test="@type='ownership'">561</xsl:when>
					<xsl:when test="@type='preferred citation'">524</xsl:when>
					<xsl:when test="@type='publications'">581</xsl:when>
					<xsl:when test="@type='reproduction'">533</xsl:when>
					<xsl:when test="@type='restriction'">506</xsl:when>
					<xsl:when test="@type='source characteristics'">786</xsl:when>
					<xsl:when test="@type='source dimensions'">786</xsl:when>
					<xsl:when test="@type='source identifier'">786</xsl:when>
					<xsl:when test="@type='source note'">786</xsl:when>
					<xsl:when test="@type='source type'">786</xsl:when>
					<xsl:when test="@type='statement of responsibility'">245</xsl:when>
					<xsl:when test="@type='system details'">538</xsl:when>
					<xsl:when test="@type='thesis'">502</xsl:when>
					<xsl:when test="@type='version identification'">562</xsl:when>

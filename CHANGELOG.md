...
- `get_field(tag)` and `get_field_or_err(tag)` accessors available on
  all three record types (`Record`, `AuthorityRecord`,
  `HoldingsRecord`) in both Rust and Python with the same call shape.
  `get_field` returns the first field with the given tag as
  `Option<&Field>` / `None`. `get_field_or_err` returns the same on
  success or raises `mrrc.FieldNotFound` (E105) with `field_tag` and
  `record_control_number` (from the 001 control field) populated for
  diagnostic context. On the Rust side, the logic lives in a default
  method on the `MarcRecord` trait; each record type exposes a thin
  inherent shim that delegates to the trait method, so Rust callers
  don't need `use mrrc::MarcRecord` in scope for the common accessor
  path while generic code over `MarcRecord` continues to work via
  trait dispatch. Existing `get_fields` (the plural slice accessor)
  is unchanged.
...
- **`SubfieldPatternQuery` negation flag** ([#63](https://github.com/dchud/mrrc/issues/63), [#73](https://github.com/dchud/mrrc/pull/73)): `SubfieldPatternQuery("020", "a", r"^978-", negate=True)` pushes pattern inversion into Rust so non-matching fields never cross the FFI boundary. Thanks to [@acdha@code4lib.social](https://code4lib.social/@acdha) for suggesting this.
- **`SubfieldValueQuery` negation flag** ([#64](https://github.com/dchud/mrrc/issues/64), [#74](https://github.com/dchud/mrrc/pull/74)): `SubfieldValueQuery("650", "a", "History", negate=True)` pushes value inversion into Rust. Works with both exact and partial matching.
- **Comprehensive pymarc API compatibility** ([#71](https://github.com/dchud/mrrc/issues/71), [#72](https://github.com/dchud/mrrc/pull/72)): Full drop-in replacement for pymarc. Thanks to [@mistersql@mastodon.social](https://mastodon.social/@mistersql) for highlighting several of these compatibility gaps. Changes include:
...
- **`SubfieldPatternQuery` and `SubfieldValueQuery` marked `#[non_exhaustive]`** ([#74](https://github.com/dchud/mrrc/pull/74)): Prevents external struct literal construction, requiring use of constructors. Future field additions will not be semver-breaking.
...
- **Serialization functions now accept wrapped Python Records** ([#57](https://github.com/dchud/mrrc/issues/57), [#58](https://github.com/dchud/mrrc/pull/58)): `record_to_json()`, `record_to_xml()`, `record_to_marcjson()`, `record_to_mods()`, `record_to_dublin_core()`, and `record_to_dublin_core_xml()` previously rejected `Record` objects returned by `xml_to_record()` and other deserialization functions with `TypeError: 'Record' object is not an instance of 'Record'`. The PyO3 functions expected the raw `_mrrc.Record` type but received the Python wrapper `mrrc.Record`. All six functions now use a shared `extract_record()` helper that accepts both types, matching the pattern already used by `record_to_csv()`.
- **`record_to_dublin_core_xml` now exported from Python wrapper** ([#59](https://github.com/dchud/mrrc/pull/59)): The function existed in the Rust extension (`_mrrc`) but was missing from `mrrc/__init__.py` imports and `__all__`, making it inaccessible as `mrrc.record_to_dublin_core_xml()`.
